...
cbuffer ParticleFxBuffer : register(b0)
{
	matrix vp;
...
struct V2G		//vertex shader to geometry shader
{
	float3 position : POSITION;
	float3 normal : NORMAL;		//used to save the dimentions
	float4 color : COLOR;
};

//----------------------------------------------------------------------------------------------------

struct G2P		//geometry shader to pixel shader
{
...
float4 PS(G2P input) : SV_Target
{
	...
	return texColor;// (input.color + texColor) * 0.5f;
}