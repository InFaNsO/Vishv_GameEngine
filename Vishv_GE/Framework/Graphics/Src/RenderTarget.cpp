...
namespace
{
	DXGI_FORMAT GetFormats(RenderTarget::Format format)
	{
		...
		default:
			VISHVASSERT(false, "[RenderTarget] Unsupported format %d.", static_cast<uint32_t>(format));
			break;
		}
...
{
	VISHVASSERT(mShaderResourceView == nullptr, "[RenderTarget] Render target not released!");
	VISHVASSERT(mRenderTargetView == nullptr, "[RenderTarget] Render target not released!");
	VISHVASSERT(mDepthStencilView == nullptr, "[RenderTarget] Render target not released!");
}
...
void RenderTarget::Initialize(uint32_t width, uint32_t height, Format format)
{
	...
	VISHVASSERT(SUCCEEDED(hr), "[RenderTarget] Failed to create render texture.");

	hr = device->CreateShaderResourceView(texture, nullptr, &mShaderResourceView);
	VISHVASSERT(SUCCEEDED(hr), "[RenderTarget] Failed to create shader resource view.");

	hr = device->CreateRenderTargetView(texture, nullptr, &mRenderTargetView);
	VISHVASSERT(SUCCEEDED(hr), "[RenderTarget] Failed to create render target view.");
	SafeRelease(texture);
	...
	VISHVASSERT(SUCCEEDED(hr), "[RenderTarget] Failed to create depth stencil texture.");

	hr = device->CreateDepthStencilView(texture, nullptr, &mDepthStencilView);
	VISHVASSERT(SUCCEEDED(hr), "[RenderTarget] Failed to create depth stencil view.");
	SafeRelease(texture);
...
{
	float ClearColor[4] = { mBGColor.r, mBGColor.g, mBGColor.b, mBGColor.a }; // RGBA
