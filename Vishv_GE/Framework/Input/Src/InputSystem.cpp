...
BOOL CALLBACK Vishv::Input::EnumGamePadCallback(const DIDEVICEINSTANCE* pDIDeviceInstance, VOID* pContext)
{
...
LRESULT CALLBACK Vishv::Input::InputSystemMessageHandler(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (sInputSystem)
	{
		switch (message)
...
{
	VISHVASSERT(sInputSystem.get() == nullptr, "[InputSystem] System already initialized!");
...
{
	VISHVASSERT(sInputSystem != nullptr, "[InputSystem] No system registered.");
...
InputSystem::~InputSystem()
{
	VISHVASSERT(!mInitialized, "[InputSystem] Terminate() must be called to clean up!");
}
...
void InputSystem::Initialize(HWND window)
{
	...
	VISHVASSERT(SUCCEEDED(hr), "[InputSystem] Failed to create DirectInput object.");
	...
	if (mGamePadDevice != nullptr)
	{
		...
		VISHVASSERT(SUCCEEDED(hr), "[InputSystem] Failed to set game pad data format.");

		// Set the game pad cooperative level
		hr = mGamePadDevice->SetCooperativeLevel(window, DISCL_FOREGROUND | DISCL_EXCLUSIVE);
		VISHVASSERT(SUCCEEDED(hr), "[InputSystem] Failed to set game pad cooperative level.");

		// Acquire the game pad device
		hr = mGamePadDevice->Acquire();
		VISHVASSERT(SUCCEEDED(hr), "[InputSystem] Failed to acquire game pad device.");
	}
	else
	{
...
{
	VISHVASSERT(mInitialized, "[InputSystem] System not initialized.");
