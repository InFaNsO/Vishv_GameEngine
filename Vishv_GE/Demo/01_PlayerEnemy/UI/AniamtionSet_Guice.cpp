...
void Vishv::UI::AnimationSetUI::BoneAnimationSeting(Vishv::Graphics::BoneAnimation * animation)
{
	...
	if (ImGui::SliderInt("Position Keys", &KeyIndexPos, 0, (int) animation->animation.GetPositionKeys().size()))
		KeyDataPosition(animation->animation.GetPositionKeys()[KeyIndexPos]);
	else if (ImGui::SliderInt("Rotation Keys", &KeyIndexPos, 0, (int)animation->animation.GetRotationKeys().size()))
		KeyDataRotation(animation->animation.GetRotationKeys()[KeyIndexPos]);
	else if (ImGui::SliderInt("Scale Keys", &KeyIndexPos, 0, (int)animation->animation.GetScaleKeys().size()))
